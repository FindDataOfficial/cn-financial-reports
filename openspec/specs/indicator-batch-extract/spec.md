# indicator-batch-extract

## Purpose

Extract many indicators for one company and fiscal year in a single pass via the `extract_indicators` tool. Computes the rules applicable to the company, fetches the annual-report PDF at most once, groups `report`-type rules by section, batches LLM extractors per section, dispatches Python extractors individually, evaluates `computed` rules locally, and caches the resulting bundle on disk.
